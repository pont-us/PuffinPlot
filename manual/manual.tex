...
\item[Calculations\submenu Anchor PCA] is a menu item which may be toggled on
  ...
  the origin. In general, it is appropriate to anchor the \caps{pca} if the
  analysed points are known to represent the final demagnetization component
  -- that is, they are trending directly towards the origin, and deviations
  from this path are known to be due to measurement noise rather than an
  offset in the true magnetization vectors. Leaving PCA unanchored allows
  analysis of non-final components, provided that they are sufficiently well
...
\item[Sample parameters] shows the results of \caps{pca} and/or great-circle
  ...
  of the PCA best-fit line. For the great-circle fit, the plot gives the
...
\item[Sample parameter table] shows a summary of parameters for each sample
  ...
  (`PCAa' and `PCAu' for anchored and unanchored PCA respectively, and `GC'
  for great circle). For PCA analysis, the declination and inclination
...
\item[Zplot PCA display] controls the manner in which PCA directions are
  shown on the Zijderveld plot (see section~\ref{sec:plot-types}). The
  available settings are:

\begin{description}
\item[Full] PCA lines will extend right to the edges of the Zijderveld
  plot.
\item[Long] PCA lines will be shortened by 10\% from the `Full' length.
\item[Short] PCAlines will only extend through the points used to calculate
  the PCA.
\item[None] No PCA lines will be shown.
\end{description}
