 ...
 * statistics and/or great-circle statistics provided that the appropriate
...
public class SiteParamsLegend extends Plot {
    ...
    /*
     * Using a real alpha character can be tricky: it should be OK if the real
     * alpha is simply absent from the font, since it should just get
     * substituted from the default font (which does contain it). But buggy
     * fonts can contain an incorrect or blank character in the alpha position,
     * so it's safer not to use it. Anyway, if we're doing it properly we should
     * subscript the 95 too.
     */
    ...
    private AttributedCharacterIterator layoutFisherParams(Graphics2D graphics,
            String name, FisherParams fp) {
        ...
        if (USE_REAL_ALPHA) {
            /*
             * A nasty little hack to deal with the alpha: if printing in a font
             * that doesn't have an alpha, the whole string will fall back to
             * the default font. So we add a dummy attribute to the alpha
             * character to make sure it's in a different style run; this means
             * that a font substitution on the alpha won't affect the whole
             * string. (See bug #32.)
             */
            final int position = s.indexOf(ALPHA);
