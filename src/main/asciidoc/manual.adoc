...
PuffinPlot paper . See
Appendix #sec:citing-puffinplot[[sec:citing-puffinplot]] for further
details.
...
details. PuffinPlot is copyright 2012–2017 by Pontus Lurcock. The
...
https://github.com/pont-us/PuffinPlot/releases. PuffinPlot is
...
section #sec:scripting[[sec:scripting]]).
Table #tbl:archive-contents[[tbl:archive-contents]] lists the contents
of the PuffinPlot archive. After downloading the archive, move it to the
folder where you would like PuffinPlot installed and extract its
contents there.

.[[tbl:archive-contents]][tbl:archive-contents] The contents of the
PuffinPlot archive file.
[cols="<,<",options="header",]
|=======================================================================
|File |Description
|README |A text file briefly describing the archive.

|PuffinPlot.jar |The PuffinPlot program (suitable for all operating
systems)
...
window, as shown in figure #fig:screenshot[[fig:screenshot]].
...
section #sec:edit-layout[[sec:edit-layout]]). Other plots are also
available, and the preferences window can be used to control which plots
are displayed (see section #sec:preferences[[sec:preferences]]).
...
==== [[sec:toolbar]][sec:toolbar]Toolbar
...
Sample orientation::
  ...
  section #sec:prefs-misc[[sec:prefs-misc]]). By default, PuffinPlot
...
Formation orientation::
  ...
  section #sec:prefs-misc[[sec:prefs-misc]]).
...
==== [[sec:menu-file]][sec:menu-file]File menu
...
suite. See section #sec:file-types[[sec:file-types]] for details of
supported filetypes.
...
Table #tbl:export-sample[[tbl:export-sample]] describes the fields which
make up the file.

.[[tbl:export-sample]][tbl:export-sample] List of fields in exported
sample data file, Note that, in addition to the predefined fields, any
custom user annotations (see
section #sec:annotations[[sec:annotations]]) will also be exported in
this file.
[cols="<,<",options="header",]
|=======================================================================
...
section #sec:plot-types[[sec:plot-types]] for more details.
...
section #sec:annotations[[sec:annotations]] for details.
|=======================================================================

saves a file containing (for suites with discrete samples) all the data
associated with sites. Table #tbl:export-site[[tbl:export-site]]
describes the fields which make up this file.

.[[tbl:export-site]][tbl:export-site] List of fields in exported site
data file
[cols="<,<",options="header",]
|=======================================================================
|Field name |Description
...
section #sec:prefs-misc[[sec:prefs-misc]] for details on the validity
...
(section #sec:functions-calcs[[sec:functions-calcs]]) for details.
...
section #sec:menu-window[[sec:menu-window]] for more details.
...
programming language. See section #sec:scripting[[sec:scripting]] for
details.

runs a user-specified external script written in the Python programming
language. See section #sec:scripting[[sec:scripting]] for details.

opens the preferences window. See
section #sec:preferences[[sec:preferences]] for details. On Mac OS X,
...
See section #sec:edit-layout[[sec:edit-layout]] for details.
...
see section #sec:annotations[[sec:annotations]] for details.
...
==== [[sec:functions-calcs]][sec:functions-calcs]Calculations menu
...
Table #tbl:export-sample[[tbl:export-sample]]).
...
are used; see section #sec:prefs-misc[[sec:prefs-misc]] for details of
...
==== [[sec:menu-window]][sec:menu-window]Window menu
...
==== [[sec:file-types]][sec:file-types]Supported file types
...
Custom formats::
  ...
  section #sec:import-data[[sec:import-data]] for details.

Note that there are different ways to read 2G data files: users are
encouraged to read section #sec:prefs-2g-import[[sec:prefs-2g-import]]
...
==== [[sec:import-data]][sec:import-data]Importing data in a custom format
...
==== [[sec:plot-types]][sec:plot-types]Available plot types
...
section #sec:preferences[[sec:preferences]]). Note that the term `plot'
...
been calculated (see
section #sec:functions-calcs[[sec:functions-calcs]]), the mean
...
valid means are shown. See section #sec:prefs-misc[[sec:prefs-misc]] for
...
type::
  ...
  or `i' for invalid. See section #sec:prefs-misc[[sec:prefs-misc]] for
...
section #sec:functions-calcs[[sec:functions-calcs]]).
...
_Import site locations…_ in Section #sec:menu-file[[sec:menu-file]] for
...
locations…_ in Section #sec:menu-file[[sec:menu-file]] for details on
...
toolbar; see section #sec:toolbar[[sec:toolbar]] for details. The
...
(see section #sec:prefs-misc[[sec:prefs-misc]]).
...
==== [[sec:edit-layout]][sec:edit-layout]Arranging the plots
...
section #sec:preferences[[sec:preferences]]). To arrange the plots,
...
menu items (see section #sec:menu-file[[sec:menu-file]]).

=== [[sec:preferences]][sec:preferences]The preferences window
...
==== [[sec:prefs-2g-import]][sec:prefs-2g-import]The 2G import tab
...
Table #tbl:import-protocols[[tbl:import-protocols]] describes the
available protocols.
...
.[[tbl:import-protocols]][tbl:import-protocols]Measurement protocols for
2G data files
[cols="<,<",options="header",]
|=======================================================================
...
section #sec:plot-types[[sec:plot-types]].

==== [[sec:prefs-misc]][sec:prefs-misc]The Misc. tab
...
section #sec:functions-calcs[[sec:functions-calcs]] for details). This
...
plot (see section #sec:plot-types[[sec:plot-types]]). The available
settings are:
...
=== [[sec:annotations]][sec:annotations]Annotations
...
=== [[sec:graphics-export]][sec:graphics-export]Exporting graphics
...
=== [[sec:command-line]][sec:command-line]Running PuffinPlot from the command line
...
-script <file>::
  ...
  Section #sec:scripting[[sec:scripting]] for more details.
-scriptlanguage::
...
=== [[sec:scripting]][sec:scripting]Scripting
...
Figure #fig:script-example-1[[fig:script-example-1]] shows a simple
...
Table #tbl:treatment-step-fields[[tbl:treatment-step-fields]].
...
== Citing PuffinPlot[[sec:citing-puffinplot]][sec:citing-puffinplot]
