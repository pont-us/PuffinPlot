...
PuffinPlot paper (FIXME: citation). See <<sec:citing-puffinplot>> for
further details.
...
Calculations → Calculate \caps{pca}::
...
Calculations → \caps{pca} anchored::
...
Calculations → \caps{ams} → Calculate bootstrap AMS::
...
Calculations → \caps{ams} → Parametric bootstrap AMS::
calculates bootstrap statistics using the bootams.py program, producing
...
Calculations → \caps{ams} → Calculate Hext on AMS::
...
Calculations → \caps{ams} → Clear AMS calculations::
