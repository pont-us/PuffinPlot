...
\fi
\begin{center}
\includegraphics[width=100mm]{figures/Fratercula_arctica.jpeg}
\end{center}
...
\item[File\submenu Import AMS\ldots] imports \caps{ams} data from a file. The
  file must be in the \caps{.asc} format produced by the \ppcmd{Safyr} and
  \caps{Susar} programs distributed with \caps{Agico} kappabridges. The
  \caps{ams} data is assigned to the appropriate samples within the suite by
  matching the sample names specified in the \caps{asc} file with the sample
  names for the demagnetization data. If the \caps{ams} file contains data
  for samples not in the suite, these samples will be created and added to
  the suite. \caps{Ams} data is not displayed by default; the equal-area plot
  of \caps{ams} data can be activated from the \ppcmd{Preferences} window.
  AMS import has been tested with data from \caps{Safyr} versions 1.5 and 2.6
  and \caps{Susar} version 4.0.
...
\item[Label samples in site plot] If this box is ticked, sample 
  directions (PCA or Fisher) will be labelled with the sample's
  name in the site equal-area plot.
...
PuffinPlot continues to be developed, albeit slowly and sporadically now that
its main functionality is stable and fairly complete. Bugs are fixed when
they are reported, and new features are added when time permits. The
PuffinPlot project, including downloadable packages of all released versions
and the complete source code, is hosted at
\url{http://code.google.com/p/puffinplot/}. Bug reports, feature suggestions,
and feedback of all kinds are always very welcome. Please send them to
