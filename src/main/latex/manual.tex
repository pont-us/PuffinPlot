...
% titlesec lets use define \sectionbreak to start sections on a new
% page. Warning: some titlesec versions have a bug that suppresses
% section numbering -- fixed in v2.10.2 (see
% https://tex.stackexchange.com/q/299969/93056 ).
\usepackage{titlesec}
...
\caps{mdf} demagnetization (degC or mT) & the treatment level
at which the intensity of the sample's remanence was reduced to half the
\caps{nrm} (in \textdegree C or mT). If half-intensity was not reached,
...
\item[Calculations\submenu MDF] calculates the Median Destructive Field (or,
  ...
  can be saved as part of the exported sample data. If the sample never
  reaches half-intensity during treatment, the MDF is undefined; in this
  case, it is not shown on the plot, and an MDF value of 0 is used in
...
\item[Calculations\submenu Great circles] calculates a best-fitting
  direction for all the great circles fitted at the sites corresponding to
  the currently selected samples, using the algorithm of
  \cite{mcfadden1988circles}. For any sample which has a PCA direction but
  no fitted great circle, the PCA direction will be used as an endpoint.
  The great-circle fit is shown both in the \ppcmd{Equal-area (site)} plot
  in the main window (if the plot has been activated in the preferences),
...
\item[Calculate RPI\ldots] calculates relative palaeointensity using two
  ...
  the RPI estimate can also be shown in the RPI plot.
...
\item[Demag.] is a plot of treatment level (in mT or \textdegree C) versus
  ...
  overlaid on the same plot as unfilled points, with the scale shown on
  the right of the graph. If PuffinPlot cannot find any data describing
  the demagnetization treatment (i.e. AF field strength or temperature),
  the X axis will be labelled `measurement number', and the X values will
  correspond to the sequence of the data in the file.
...
\item[RPI Plot] plots the current RPI estimate (if any) against depth.
...
\item[VGP map] is a world map (using a Mollweide projection) showing the
  locations of virtual geomagnetic poles (VGPs) for each site in the
  current suite. VGPs can only be calculated for sites whose location has
  been set; See \emph{Import site locations\ldots} in
  Section~\ref{sec:menu-file} for details on reading site locations from a
  \caps{csv} file.
...
\begin{table}
\makebox[\textwidth][c]{
\begin{tabular}{lllp{10cm}} \toprule
Identifier & Type / Unit & Default & Description \\ \midrule
\ppcmd{DISCRETE\_ID} & string & \ppcmd{UNSET} & The discrete sample identifier (sample name). \\
\ppcmd{DEPTH} & unspecified & null &
...
\ppcmd{VOLUME} & cm${}^3$ & 10.8 & The volume of a discrete sample. \\
\ppcmd{AREA} & cm${}^2$ & 4 & The cross-sectional area of a long core sample. \\
\ppcmd{SAMPLE\_AZ} & \textdegree{} & NaN & The azimuth (i.e. down-dip direction) of a sample's field orientation. \\
...
% Changelog is automatically generated from Markdown source.
\input{changes.tex}
